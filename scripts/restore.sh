...
#
# Usage: bash scripts/restore.sh backups/iris_backup_YYYYMMDD_HHMMSS.tar.gz
#
...
BACKUP_FILE="${1:-}"
TEMP_DIR="/tmp/iris_restore_$(date +%s)"

if [ -z "${BACKUP_FILE}" ]; then
  echo "ERROR: Please provide a backup file path."
  echo "Usage: bash scripts/restore.sh backups/iris_backup_YYYYMMDD_HHMMSS.tar.gz"
  exit 1
fi
...
echo "=== IRIS Restore Started from: ${BACKUP_FILE} ==="
